...
classification_results_all = r'D:\WorkStation_2018\SZ_classification\Data\ML_data_npy\results_fc_excluded_greater_fd_and_regressed_out_site_age_sex_motion_all.npy'
classification_results_separately = r'D:\WorkStation_2018\SZ_classification\Data\ML_data_npy\results_fc_excluded_greater_fd_and_regressed_out_age_sex_motion_separately.npy'
...
#%% Statistics
# all
n = len(scale_550_selected_all[1].values)
acc = accuracy_all
k = np.int32(n * acc)
...
# Filter subjects that have .mat files
scale_550_selected_separatly = pd.merge(results_special, scale_550, left_on=0, right_on='folder', how='inner')
...
data_chronic_medicated_SSD_550_18_separatly = scale_550_selected_separatly[
    ...
    (scale_550_selected_separatly['病程月'] > duration) & 
    (scale_550_selected_separatly['用药'] == 1)
]
data_firstepisode_medicated_SSD_550_18_separatly = scale_550_selected_separatly[
    (scale_550_selected_separatly['诊断']==3) & 
    (scale_550_selected_separatly['首发'] == 1) &
    (scale_550_selected_separatly['病程月'] <= duration) & 
    (scale_550_selected_separatly['用药'] == 1)
]
data_firstepisode_unmedicated_SSD_550_18_separatly = scale_550_selected_separatly[
    (scale_550_selected_separatly['诊断']==3) & 
    (scale_550_selected_separatly['首发'] == 1) &
    (scale_550_selected_separatly['病程月'] <= duration) & 
    (scale_550_selected_separatly['用药'] == 0)
]

#%% Calculating Accuracy
# all
acc_chronic_medicated_SSD_550_18_separatly = np.sum(data_chronic_medicated_SSD_550_18_separatly[1]-data_chronic_medicated_SSD_550_18_separatly[3]==0) / len(data_chronic_medicated_SSD_550_18_separatly)
acc_firstepisode_medicated_SSD_550_18_separatly = np.sum(data_firstepisode_medicated_SSD_550_18_separatly[1]-data_firstepisode_medicated_SSD_550_18_separatly[3]==0) / len(data_firstepisode_medicated_SSD_550_18_separatly)
acc_first_episode_unmedicated_SSD_550_18_separatly = np.sum(data_firstepisode_unmedicated_SSD_550_18_separatly[1]-data_firstepisode_unmedicated_SSD_550_18_separatly[3]==0) / len(data_firstepisode_unmedicated_SSD_550_18_separatly)
accuracy, sensitivity, specificity, auc = eval_performance(scale_550_selected_separatly[1].values, scale_550_selected_separatly[3].values, scale_550_selected_separatly[2].values,
                    accuracy_kfold=None, sensitivity_kfold=None, specificity_kfold=None, AUC_kfold=None,
                    verbose=True, is_showfig=False, legend1='HC', legend2='Patients', is_savefig=False, out_name=None)

#%% Statistics
n = len(scale_550_selected_separatly[1].values)
acc = accuracy
k = np.int32(n * acc)
p, sum_prob, prob, randk = lc_binomialtest(n, k, 0.5, 0.5)
print(p)
n = len(data_chronic_medicated_SSD_550_18_separatly)
acc = acc_chronic_medicated_SSD_550_18_separatly
k = np.int32(n * acc)
p, sum_prob, prob, randk = lc_binomialtest(n, k, 0.5, 0.5)
print(p)
n = len(data_firstepisode_medicated_SSD_550_18_separatly)
acc = acc_firstepisode_medicated_SSD_550_18_separatly
k = np.int32(n * acc)
p, sum_prob, prob, randk = lc_binomialtest(n, k, 0.5, 0.5)
print(p)
n = len(data_firstepisode_unmedicated_SSD_550_18_separatly)
acc = acc_first_episode_unmedicated_SSD_550_18_separatly
k = np.int32(n * acc)
p, sum_prob, prob, randk = lc_binomialtest(n, k, 0.5, 0.5)
print(p)
...
ax2 = plt.bar(
    ...
    [
        ...
        sensitivity, 
        specificity,
        acc_chronic_medicated_SSD_550_18_separatly,
        acc_firstepisode_medicated_SSD_550_18_separatly, 
        acc_first_episode_unmedicated_SSD_550_18_separatly
    ], 
    color='paleturquoise',
    alpha=1)
plt.grid(axis='y')
